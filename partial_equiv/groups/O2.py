...
class E2(Group):
    ...
    def product(self, g1, g2):
        ...
        rot = torch.remainder(
            g2[..., 0].unsqueeze(-2)
            + (g2[..., 1].unsqueeze(-2) * g1[..., 0].unsqueeze(-1)),
            2 * np.pi,
        )
        mirror = g1[..., 1].unsqueeze(-1) * g2[..., 1].unsqueeze(-2)

        # rot = torch.remainder(g2[..., 0].unsqueeze(-2) + (g2[..., 1].unsqueeze(-2) * g1[..., 0].unsqueeze(-1)),  2 * np.pi)
        # mirror = g1[..., 1].unsqueeze(-1) * g2[..., 1].unsqueeze(-2)
    ...
    def left_action_on_Rd(self, g, x):
        ...
        """
        return torch.einsum("noi,iyx->noyx", self.matrix_form(g), x.double()).type(
            torch.float32
        )
        ...
        if method == SamplingMethods.DETERMINISTIC:
            ...
            if not partial_equivariance:
                ...
                uniform_grid_rot = torch.linspace(
                    ...
                    2
                    * math.pi
                    * float(no_elements_rotation - 1)
                    / float(no_elements_rotation),
                ...
                # Concatenation
                grid = torch.stack(
                    [uniform_grid_rot.repeat(1, 2), grid_reflexion], dim=-1
                )
                ...
                if unnormalized_prob_rotation.numel() == 0:
                    ...
                    prob_rotations = (
                        torch.distributions.RelaxedBernoulli(
                            ...
                            probs=torch.sigmoid(
                                self.sigmoid_temp * unnormalized_prob_rotation
                            ),
                        )
                        .rsample([1])
                        .squeeze()
                    ...
                    sample_rotation = (
                        sample_rotation - prob_rotations.detach() + prob_rotations
                    )

                    # Concatenate a 1.0 at the beginning (the probability of the identity)
                    sample_rotation = torch.cat(
                        [torch.ones(1, device=device), sample_rotation]
                    )
                ...
                prob_reflexion = torch.distributions.RelaxedBernoulli(
                    ...
                    probs=torch.sigmoid(
                        self.sigmoid_temp * unnormalized_prob_reflexion
                    ),
                ...
                sample_reflexion = (
                    sample_reflexion - prob_reflexion.detach() + prob_reflexion
                )
            ...
            if not partial_equivariance:
                ...
                delta = (
                    torch.rand(no_samples, 1, device=device)
                    * (2 * math.pi)
                    / float(no_elements_rotation)
                )
                ...
                no_elements_rotation = min(
                    math.ceil(no_elements_rotation * theta.item()), no_elements_rotation
                )
                ...
                uniform_grid_rot = (uniform_grid_rot - math.pi * theta).unsqueeze(
                    0
                ) + delta
    ...
    def get_current_gumbel_temperature(self):
        current_temperature = (
            self.gumbel_init_temperature
            - self.gumbel_iter_counter
            / float(self.gumbel_no_iterations)
