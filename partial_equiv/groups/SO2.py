...
class SE2(Group):
    ...
    def left_action_on_Rd(self, g, x):
        ...
        """
        return torch.einsum("noi,ixy->noxy", self.matrix_form(g), x.double()).type(
            torch.float32
        )
    ...
    def sample_from_stabilizer(
        ...
        if method == SamplingMethods.DETERMINISTIC:
            ...
            if not partial_equivariance:
                ...
                no_elements = min(
                    math.ceil(no_elements * abs(probs.item())), no_elements
                )
                ...
                g_elems = self.exponential_map(
                    (uniform_grid - math.pi * probs).unsqueeze(0) + delta
                )
